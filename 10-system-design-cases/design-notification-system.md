...
- (1) API to trigger notifications.
- (2) Template engine.
- (3) Channels: push (FCM/APNS), email (SES/SendGrid), in-app (WebSocket or poll).
- (4) User preferences store (channels, frequency).
- (5) Queue for async processing.

Delivery: retries, dead-letter, idempotency. Preferences: check before send; respect opt-out and frequency limits. Batch for email to reduce cost.
